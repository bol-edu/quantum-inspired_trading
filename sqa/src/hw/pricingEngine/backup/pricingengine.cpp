...
void PricingEngine::pricingProcess(ap_uint<32> &regStrategyControl, ap_uint<32> &regProcessResponse,
                                   ...
                                   ap_uint<32> &regResv0, ap_uint<32> &regResv1,
                                   ap_uint<32> &regResv2, ap_uint<32> &regResv3,
    ...
    if (!responseStream.empty()) {
        ...
        for (int i = 0; i < PHYSICAL_BITS; i++) {
            for (int j = 0; j < PHYSICAL_BITS; j++) {
                J[i][j] = (fp_t)(i) * (fp_t)(j) / PHYSICAL_BITS / PHYSICAL_BITS;
            }
            h[i] = 0;
        }
#endif
        ...
        // RUN SQA
        runSQA(spins, J, h, regResv0, regResv1, regResv2, regResv3);

#if !__SYNTHESIS__ && 0
        fp_t  a = 0, b = 0;
        for (int i = 0; i < PHYSICAL_BITS; i++) {
            fp_t tmp = (fp_t) i / PHYSICAL_BITS;;
            if (spins[i]) {
                a += tmp;
            } else {
                b += tmp;
...
void PricingEngine::runSQA(spin_t spins[NUM_SPIN], float J[NUM_SPIN][NUM_SPIN], float h[NUM_SPIN], 
                           ap_uint<32> &resv0, ap_uint<32> &resv1, ap_uint<32> &resv2, ap_uint<32> &resv3)
{
    // Internal Trotters
...
#pragma HLS PIPELINE off
    ...
    for (int i = 0; i < PHYSICAL_BITS; i++) { resv0[i] = trotters[0][i]; }
    for (int i = 0; i < PHYSICAL_BITS; i++) { resv1[i] = trotters[1][i]; }
    for (int i = 0; i < PHYSICAL_BITS; i++) { resv2[i] = trotters[2][i]; }
    for (int i = 0; i < PHYSICAL_BITS; i++) { resv3[i] = trotters[3][i]; }
    ...
    resv0 = 0xcafebabe;
    resv1[31] = !tmp;
    resv2.range(31, 24) = run_count;
    resv2.range(23, 16) = run_count;
    resv2.range(15,  8) = run_count;
    resv2.range( 7,  0) = run_count;
    resv3 = 0xdeadbeef; 
}
